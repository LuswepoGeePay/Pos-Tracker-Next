...
const CreatePosDeviceSchema = z.object({
    name: z.string(),
...
const CreatePosDeviceForm = ({ appid }: { appid: string }) => {
    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const { data: session } = useSession()

    const form = useForm<z.infer<typeof CreatePosDeviceSchema>>({
    ...
    const onSubmit = async (values: z.infer<typeof CreatePosDeviceSchema>) => {
        try {
            setLoading(true)
            ...
            const response = await fetch(api_endpoints.createPosDevice, {
                method: 'POST',
                headers: {
...
export default CreatePosDeviceForm